...
const loadBenefit = async () => {
    const response = await benefitStore.show(route.params.id);
    form.title = response.data.title;
    form.description = response.data.description;
    form.provider = response.data.provider;
    form.video_id = response.data.video_id;
    form.sort_order = response.data.sort_order;
    form.banner_url = response.data.banner_url;
}
...
const submit = async () => {
    ...
    await benefitStore.update(route.params.id, formData)
}
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div class="col-span-2">
                        <form @submit.prevent="submit" enctype="multipart/form-data" class="max-w-2xl">
                            <BaseInput id="title" label="Title" v-model="form.title" name="title"
                                placeholder="Enter title" :error="errors.title" :required="true" />

                            <BaseTextarea id="description" label="Description" v-model="form.description"
                    ...
                    <div class="flex-none border border-border p-2 rounded-xl">
                        <img :src="form.banner_url" alt="banner" class="h-32 w-auto" />
                    </div>
                </div>
            </div>
        </div>
